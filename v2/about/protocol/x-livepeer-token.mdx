...
description: >-
  Livepeer Token (LPT) is the native proof-of-stake token of the Livepeer network. It is the key to participating in the
  protocol: "to operate or delegate in the transcoding network, to earn rewards, and to vote on governance proposals."
  This page details the technical attributes, token supply dynamics, inflation model, and contract integrations that
  define LPT’s role.
...
<DynamicTable
  tableTitle={<Subtitle style={{fontSize: '0.9rem'}}>LPT Usage</Subtitle>}
  ...
  itemsList={[
    ...
    { "Use Case": "Slashing Guarantee", "LPT Functionality": "Orchestrators risk LPT loss for malicious behaviour" },
