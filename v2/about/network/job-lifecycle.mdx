...
description: >-
  This page describes the end-to-end “compute job” as a state machine. Since Livepeer’s compute is segment-oriented, the
  lifecycle is modelled at the level of a stream session and per-segment processing, with payment settlement occurring
  continuously via tickets and periodically via reward calls.
...
flowchart LR
 ...
 Gateway -->|“next segment / finalise”| Orchestrator
...
Example (AI): A user feeds a webcam stream into Daydream (powered by Livepeer AI). The Gateway sends frames plus a “style” prompt to Orchestrator B. B runs a sequence of GPUs (e.g. Enhance → stylize) and returns an AI-edited video with low latency. Livepeer’s GPUs and networking are optimised for this low-latency pipeline
.
