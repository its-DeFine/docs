...
'og:image': /snippets/assets/site/og-image/en/page-blockchain-contracts.png
...
export const arb = contractAddresses.meta.explorerUrls.arbiscanAddress || `${contractAddresses.meta.explorerUrls.arbiscan || 'https://arbiscan.io'}/address/`
export const eth = contractAddresses.meta.explorerUrls.etherscanAddress || `${contractAddresses.meta.explorerUrls.etherscan || 'https://etherscan.io'}/address/`

export const dedupeContracts = (entries = []) => {
  const seen = new Set()
  return entries.filter((entry) => {
    const key = `${entry?.name || ''}:${entry?.type || entry?.deploymentKind || ''}:${entry?.address || ''}`.toLowerCase()
...
export const chainPayload = (chain) => contractAddresses[chain] || {}
export const chainActiveContracts = (chain) => dedupeContracts(chainPayload(chain).active || chainPayload(chain).current || [])
export const chainImplementationContracts = (chain) => dedupeContracts(
  chainPayload(chain).currentImplementations || (chainPayload(chain).current || []).filter((entry) => (entry.type || entry.deploymentKind) === 'target')
)
export const chainInventoryContracts = (chain) => {
  const payload = chainPayload(chain)
  return dedupeContracts([
    ...(payload.inventory || []),
    ...chainActiveContracts(chain),
    ...(payload.paused || []),
    ...(payload.migration_residual || []),
    ...(payload.legacy_operational || []),
    ...chainImplementationContracts(chain),
  ])
}

export const findContractEntry = (entries, name, type = null) =>
  (entries || []).find((entry) =>
    entry?.name === name && (type == null || (entry.type || entry.deploymentKind) === type)
  )

export const getActiveEntry = (chain, name, type = null) => findContractEntry(chainActiveContracts(chain), name, type)
export const getImplementationEntry = (chain, name) => findContractEntry(chainImplementationContracts(chain), name, 'target')
export const getPublishedEntry = (chain, name, type = null) =>
  getActiveEntry(chain, name, type) ||
  getImplementationEntry(chain, name) ||
  findContractEntry(chainInventoryContracts(chain), name, type)

export const getCodeHref = (chain, name, type = null) => getPublishedEntry(chain, name, type)?.contractCodeHref || null
export const toRawGitHubUrl = (href) => href
  ? href.replace('https://github.com/', 'https://raw.githubusercontent.com/').replace('/blob/', '/')
  : null
export const getExplorerHref = (chain, name, type = null) => {
  const entry = getPublishedEntry(chain, name, type)
  if (!entry?.address) return null
  return entry.blockchainHref || `${chain === 'ethereumMainnet' ? eth : arb}${entry.address}`
}
export const getRawCodeHref = (chain, name, type = null) => toRawGitHubUrl(getCodeHref(chain, name, type))

export const controller = getActiveEntry('arbitrumOne', 'Controller')?.address
export const bondingManagerProxy = getActiveEntry('arbitrumOne', 'BondingManager', 'proxy')?.address
export const ticketBrokerProxy = getActiveEntry('arbitrumOne', 'TicketBroker', 'proxy')?.address
export const roundsManagerProxy = getActiveEntry('arbitrumOne', 'RoundsManager', 'proxy')?.address
export const minter = getActiveEntry('arbitrumOne', 'Minter')?.address
export const serviceRegistryProxy = getActiveEntry('arbitrumOne', 'ServiceRegistry', 'proxy')?.address
export const aiServiceRegistry = getActiveEntry('arbitrumOne', 'AIServiceRegistry')?.address
export const lptArb = getActiveEntry('arbitrumOne', 'LivepeerToken')?.address
export const lptEth = getActiveEntry('ethereumMainnet', 'LivepeerToken')?.address
export const bridgeMinter = getActiveEntry('ethereumMainnet', 'BridgeMinter')?.address
export const l2Gateway = getActiveEntry('arbitrumOne', 'L2LPTGateway')?.address
export const l1Gateway = getActiveEntry('ethereumMainnet', 'L1LPTGateway')?.address
export const l1Escrow = getActiveEntry('ethereumMainnet', 'L1Escrow')?.address
export const bondingVotesProxy = getActiveEntry('arbitrumOne', 'BondingVotes', 'proxy')?.address
export const governor = getActiveEntry('arbitrumOne', 'Governor')?.address
export const livepeerGovernorProxy = getActiveEntry('arbitrumOne', 'LivepeerGovernor', 'proxy')?.address
export const treasury = getActiveEntry('arbitrumOne', 'Treasury')?.address
export const l2Migrator = getPublishedEntry('arbitrumOne', 'L2Migrator', 'proxy')?.address
export const merkleSnapshot = getPublishedEntry('arbitrumOne', 'MerkleSnapshot')?.address
...
<Tip>
  ...
  <span style={{display: "flex", alignItems: "center", marginTop: "0.2rem"}}> Data is sourced from the <DoubleIconLink label="Livepeer Github" href="https://github.com/livepeer/" iconRightColor="var(--hero-text)"/> and verified on-chain. </span> <br/>
  <span style={{color: "var(--text)", marginTop: "0.5rem"}}> Last Verified: <DataWrap value={contractAddresses.meta.lastVerified} /> </span>
</Tip>
...
<Accordion  title={<CustomCardTitle variant="accordion" icon={<ArbitrumIcon color="var(--arbitrum)" />} title="Controller"/>} >
  <AddressLinks
  address={controller}
  blockchainHref={getExplorerHref('arbitrumOne', 'Controller')}
  githubHref={getCodeHref('arbitrumOne', 'Controller')}
/>
{/* CHANGELOG PIPELINE UPDATE */}
<Icon icon="check" color="var(--text)"/> _Verified · Active · Deployed by Livepeer Deployer_
...
<SolidityEmbed
  url={toRawGitHubUrl(getCodeHref('arbitrumOne', 'Controller'))}
...
<Accordion title={<CustomCardTitle variant="accordion" icon={<ArbitrumIcon color="var(--arbitrum)" />} title="BondingManager" />}>
  **Address (Arbitrum One)**:
  <AddressLinks address={bondingManagerProxy} blockchainHref={getExplorerHref('arbitrumOne', 'BondingManager', 'proxy')} githubHref={getCodeHref('arbitrumOne', 'BondingManager', 'proxy')} />
  {/* CHANGELOG PIPELINE UPDATE */}
...
<SolidityEmbed
  url={toRawGitHubUrl(getCodeHref('arbitrumOne', 'BondingManager', 'proxy'))}
...
<Accordion title={<CustomCardTitle variant="accordion" icon={<ArbitrumIcon color="var(--arbitrum)" />} title="TicketBroker" />}>
  **Address (Arbitrum One)**:
  <AddressLinks address={ticketBrokerProxy} blockchainHref={getExplorerHref('arbitrumOne', 'TicketBroker', 'proxy')} githubHref={getCodeHref('arbitrumOne', 'TicketBroker', 'proxy')} />
  {/* CHANGELOG PIPELINE UPDATE */}
<Icon icon="check" color="var(--text)"/> _Verified · Active_
...
<SolidityEmbed
  url={toRawGitHubUrl(getCodeHref('arbitrumOne', 'TicketBroker', 'proxy'))}
...
<Accordion title={<CustomCardTitle variant="accordion" icon={<ArbitrumIcon color="var(--arbitrum)" />} title="RoundsManager" />}>
  **Address (Arbitrum One)**:
  <AddressLinks address={roundsManagerProxy} blockchainHref={getExplorerHref('arbitrumOne', 'RoundsManager', 'proxy')} githubHref={getCodeHref('arbitrumOne', 'RoundsManager', 'proxy')} />
  {/* CHANGELOG PIPELINE UPDATE */}
<Icon icon="check" color="var(--text)"/> _Verified · Active_
...
<SolidityEmbed
  url={toRawGitHubUrl(getCodeHref('arbitrumOne', 'RoundsManager', 'proxy'))}
...
<Accordion title={<CustomCardTitle variant="accordion" icon={<ArbitrumIcon color="var(--arbitrum)" />} title="Minter" />}>
  **Address (Arbitrum One)**:
  <AddressLinks address={minter} blockchainHref={getExplorerHref('arbitrumOne', 'Minter')} githubHref={getCodeHref('arbitrumOne', 'Minter')} />
  {/* CHANGELOG PIPELINE UPDATE */}
<Icon icon="check" color="var(--text)"/> _Verified · Active_
...
<SolidityEmbed
  url={toRawGitHubUrl(getCodeHref('arbitrumOne', 'Minter'))}
...
<Accordion title={<CustomCardTitle variant="accordion" icon={<ArbitrumIcon color="var(--arbitrum)" />} title="ServiceRegistry" />}>
  **Address (Arbitrum One)**:
  <AddressLinks address={serviceRegistryProxy} blockchainHref={getExplorerHref('arbitrumOne', 'ServiceRegistry', 'proxy')} githubHref={getCodeHref('arbitrumOne', 'ServiceRegistry', 'proxy')} />
  {/* CHANGELOG PIPELINE UPDATE */}
...
<SolidityEmbed
  url={toRawGitHubUrl(getCodeHref('arbitrumOne', 'ServiceRegistry', 'proxy'))}
...
<Accordion title={<CustomCardTitle variant="accordion" icon={<ArbitrumIcon color="var(--arbitrum)" />} title="AIServiceRegistry" />}>
  **Address (Arbitrum One)**:
  <AddressLinks address={aiServiceRegistry} blockchainHref={getExplorerHref('arbitrumOne', 'AIServiceRegistry')} githubHref={getCodeHref('arbitrumOne', 'AIServiceRegistry')} />
  {/* CHANGELOG PIPELINE UPDATE */}
...
<Accordion title={<CustomCardTitle variant="accordion" icon={<LivepeerIcon color="var(--accent)" size={13} />} title="LivepeerToken" />}>
  **Address (Arbitrum One)**:
  <AddressLinks address={lptArb} blockchainHref={getExplorerHref('arbitrumOne', 'LivepeerToken')} githubHref={getCodeHref('arbitrumOne', 'LivepeerToken')} />
  {/* CHANGELOG PIPELINE UPDATE */}
<Icon icon="check" color="var(--text)"/> _Verified · 261,217 holders · Active_

**Address (Ethereum Mainnet)**:

  <AddressLinks address={lptEth} blockchainHref={getExplorerHref('ethereumMainnet', 'LivepeerToken')} githubHref={getCodeHref('ethereumMainnet', 'LivepeerToken')} />
  {/* CHANGELOG PIPELINE UPDATE */}
...
<SolidityEmbed
  url={toRawGitHubUrl(getCodeHref('arbitrumOne', 'LivepeerToken'))}
...
<Info>
  ...
  <span style={{display: "flex", alignItems: "center", marginTop: "0.5rem"}}> See <DoubleIconLink label="BridgeMinter.sol" href={getCodeHref('ethereumMainnet', 'BridgeMinter')}/> on the `streamflow` branch of `livepeer/protocol`</span>
</Info>

<Accordion title={<CustomCardTitle variant="accordion" icon={<Icon icon="ethereum" color="var(--hero-text)"/>} title="BridgeMinter" />}>
  **Address (Ethereum Mainnet)**:
  <AddressLinks address={bridgeMinter} blockchainHref={getExplorerHref('ethereumMainnet', 'BridgeMinter')} githubHref={getCodeHref('ethereumMainnet', 'BridgeMinter')} />
...
<SolidityEmbed
  url={toRawGitHubUrl(getCodeHref('ethereumMainnet', 'BridgeMinter'))}
...
<Accordion title={<CustomCardTitle variant="accordion" icon={<ArbitrumIcon color="var(--arbitrum)" />} title="Bridge Gateways" />}>
  **L2LPTGateway (Arbitrum One)**:
  <AddressLinks address={l2Gateway} blockchainHref={getExplorerHref('arbitrumOne', 'L2LPTGateway')} githubHref={getCodeHref('arbitrumOne', 'L2LPTGateway')} />
  {/* CHANGELOG PIPELINE UPDATE */}
<Icon icon="check" color="var(--text)"/> _Verified · Active_

**L1LPTGateway (Ethereum Mainnet)**:

  <AddressLinks address={l1Gateway} blockchainHref={getExplorerHref('ethereumMainnet', 'L1LPTGateway')} githubHref={getCodeHref('ethereumMainnet', 'L1LPTGateway')} />
  {/* CHANGELOG PIPELINE UPDATE */}
<Icon icon="check" color="var(--text)"/> _Verified · Active_

**L1Escrow (Ethereum Mainnet)**:

  <AddressLinks address={l1Escrow} blockchainHref={getExplorerHref('ethereumMainnet', 'L1Escrow')} githubHref={getCodeHref('ethereumMainnet', 'L1Escrow')} />
  {/* CHANGELOG PIPELINE UPDATE */}
...
<SolidityEmbed
  url={toRawGitHubUrl(getCodeHref('arbitrumOne', 'L2LPTGateway'))}
...
<Accordion title={<CustomCardTitle variant="accordion" icon={<ArbitrumIcon color="var(--arbitrum)" />} title="BondingVotes" />}>
  **Address (Arbitrum One)**:
  <AddressLinks address={bondingVotesProxy} blockchainHref={getExplorerHref('arbitrumOne', 'BondingVotes', 'proxy')} githubHref={getCodeHref('arbitrumOne', 'BondingVotes', 'proxy')} />
  {/* CHANGELOG PIPELINE UPDATE */}
<Icon icon="check" color="var(--text)"/> _Verified · Active_
...
<SolidityEmbed
  url={toRawGitHubUrl(getCodeHref('arbitrumOne', 'BondingVotes', 'proxy'))}
...
<Accordion title={<CustomCardTitle variant="accordion" icon={<ArbitrumIcon color="var(--arbitrum)" />} title="Governor" />}>
  **Address (Arbitrum One)**:
  <AddressLinks address={governor} blockchainHref={getExplorerHref('arbitrumOne', 'Governor')} githubHref={getCodeHref('arbitrumOne', 'Governor')} />
  {/* CHANGELOG PIPELINE UPDATE */}
<Icon icon="check" color="var(--text)"/> _Verified · Active · Deployed by Livepeer Deployer_
...
<SolidityEmbed
  url={toRawGitHubUrl(getCodeHref('arbitrumOne', 'Governor'))}
...
<Accordion title={<CustomCardTitle variant="accordion" icon={<ArbitrumIcon color="var(--arbitrum)" />} title="LivepeerGovernor" />}>
  **Address (Arbitrum One)**:
  <AddressLinks address={livepeerGovernorProxy} blockchainHref={getExplorerHref('arbitrumOne', 'LivepeerGovernor', 'proxy')} githubHref={getCodeHref('arbitrumOne', 'LivepeerGovernor', 'proxy')} />
  {/* CHANGELOG PIPELINE UPDATE */}
<Icon icon="check" color="var(--text)"/> _Verified · Active_
...
<SolidityEmbed
  url={toRawGitHubUrl(getCodeHref('arbitrumOne', 'LivepeerGovernor', 'proxy'))}
...
<Accordion title={<CustomCardTitle variant="accordion" icon={<ArbitrumIcon color="var(--arbitrum)" />} title="Treasury" />}>
  **Address (Arbitrum One)**:
  <AddressLinks address={treasury} blockchainHref={getExplorerHref('arbitrumOne', 'Treasury')} githubHref={getCodeHref('arbitrumOne', 'Treasury')} />
  {/* CHANGELOG PIPELINE UPDATE */}
<Icon icon="check" color="var(--text)"/> _Verified · Active_
...
<SolidityEmbed
  url={toRawGitHubUrl(getCodeHref('arbitrumOne', 'Treasury'))}
...
<Accordion title={<CustomCardTitle variant="accordion" icon={<ArbitrumIcon color="var(--arbitrum)" />} title="L2Migrator" />}>
  **Address (Arbitrum One)**:
  <AddressLinks address={l2Migrator} blockchainHref={getExplorerHref('arbitrumOne', 'L2Migrator', 'proxy')} githubHref={getCodeHref('arbitrumOne', 'L2Migrator', 'proxy')} />
  <Icon icon="check" color="var(--text)"/> _Verified · Registered in Controller · Last active Feb 2026_
...
<Accordion title={<CustomCardTitle variant="accordion" icon={<ArbitrumIcon color="var(--arbitrum)" />} title="MerkleSnapshot" />}>
  **Address (Arbitrum One)**:
  <AddressLinks address={merkleSnapshot} blockchainHref={getExplorerHref('arbitrumOne', 'MerkleSnapshot')} githubHref={getCodeHref('arbitrumOne', 'MerkleSnapshot')} />
  <Icon icon="check" color="var(--text)"/> _Verified · Registered in Controller · No activity since deployment_
...
<SolidityEmbed
  url={toRawGitHubUrl(getCodeHref('arbitrumOne', 'MerkleSnapshot'))}
...
<Icon icon="check" color="var(--text)"/> <span style={{fontStyle: "italic", color: "var(--text)"}}>Last Verified {<DataWrap value={contractAddresses.meta.lastVerified} />}</span>
