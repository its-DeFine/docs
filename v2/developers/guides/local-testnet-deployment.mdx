...
<Tip>
  This guide covers local Hardhat deployment only. For Arbitrum Sepolia testnet deployment the same deploy script applies — swap the Hardhat network target for `arbitrumSepolia` and provide a funded Sepolia wallet.
</Tip>
