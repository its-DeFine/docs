...
<Steps>
  ...
  <Step title="Start a local Anvil chain">
  ...
  <Step title="Avoid the bundled Hardhat node for go-livepeer">
  ...
  <Step title="Set fast local rounds, unpause, and note the Controller address">
...
<Steps>
  ...
  <Step title="Create local-only keystores">
  ...
  <Step title="Start an orchestrator node">
    Replace `<CONTROLLER_ADDRESS>` with the address from the previous step.
