...
sidebarTitle: BYOC
description: >-
  Step-by-step guide to building a custom AI container using the PyTrickle integration layer and deploying it as a
  worker on the Livepeer network.
...
livepeer <orchestrator flags> <byoc registration flags>
```
...
```bash icon="terminal"
docker pull <current-comfystream-image>:<tag>
```
