...
Bonding is non-custodial: delegators' LPT remains within the BondingManager. To withdraw, a delegator initiates an unbond and waits for the protocol-defined thawing period before withdrawal becomes available. This unbonding period prevents rapid stake flipping and provides time for slashing (when enabled) to be enforced.
...
Because inflation is distributed continuously, delegators benefit from regularly compounding their earnings by calling `claimEarnings()` or by bonding additional LPT. Failure to claim for many rounds does not forfeit rewards, but it can delay compounding and reduce effective yield due to dilution.
