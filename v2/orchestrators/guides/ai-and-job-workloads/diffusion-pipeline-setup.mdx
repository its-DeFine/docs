...
description: >-
  Complete operator guide for running batch AI inference pipelines on a Livepeer orchestrator — aiModels.json
  configuration, all supported pipeline types, Ollama LLM runner deployment, BYOC external containers, and
  troubleshooting.
...
```json icon="terminal" title="Minimal aiModels.json example"
[
  {
...
```json icon="terminal" title="text-to-image aiModels.json entry"
{
  "pipeline": "text-to-image",
  "model_id": "SG161222/RealVisXL_V4.0_Lightning",
  "price_per_unit": 4768371,
...
```json icon="terminal" title="image-to-image aiModels.json entry"
{
...
```json icon="terminal" title="image-to-video aiModels.json entry"
{
...
```json icon="terminal" title="image-to-text aiModels.json entry"
{
...
```json icon="terminal" title="audio-to-text aiModels.json entry"
{
...
```json icon="terminal" title="segment-anything-2 aiModels.json entry"
{
...
```json icon="terminal" title="upscale aiModels.json entry"
{
  "pipeline": "upscale",
...
```json icon="terminal" title="text-to-speech aiModels.json entry"
{
...
```bash icon="terminal" title="Create the Ollama model volume"
docker volume create ollama
```
</StyledStep>

<StyledStep title="Create docker-compose.yml">
```yaml icon="terminal" title="docker-compose.yml"
services:
  ollama-ai-runner:
...
```json icon="terminal" title="LLM aiModels.json entry"
{
  "pipeline": "llm",
...
```json icon="terminal" title="SFAST optimization flag"
...
```json icon="terminal" title="DEEPCACHE optimization flag"
...
```json icon="terminal" title="Multi-pipeline aiModels.json example"
[
  {
    "pipeline": "text-to-image",
    "model_id": "SG161222/RealVisXL_V4.0_Lightning",
...
```json icon="terminal" title="BYOC audio-to-text aiModels.json entry"
{
  "pipeline": "audio-to-text",
  "model_id": "openai/whisper-large-v3",
  "price_per_unit": 12882811,
...
```json icon="terminal" title="Wei AI pricing example"
{
  "pipeline": "text-to-image",
  "model_id": "SG161222/RealVisXL_V4.0_Lightning",
  "price_per_unit": 4768371
...
```json icon="terminal" title="USD AI pricing example"
...
{/*
  REVIEW:
  - Confirm current Ollama support list with Cloud SPE or j0sh. The table above is based on the Ollama public model library.
  - Confirm the BYOC path for custom-container registration.
