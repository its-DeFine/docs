---
mode: frame
title: Welcome to Livepeer Mission Control
description: >-
  Livepeer powers on-demand real-time video streaming and AI pipelines through a full-stack open hardware network,
  providing users with affordable, verifiable, permissionless enterprise-grade software solutions.
lifecycleStage: discover
complexity: intermediate
pageType: landing
audience: everyone
purpose: landing
sidebarTitle: Mission Control
tag: Start Here
lastVerified: '2026-03-17'
keywords:
  - home
  - index
  - landing
  - livepeer
  - real-time AI
  - Daydream
'og:image': 'https://raw.githubusercontent.com/livepeer/docs/docs-v2-assets/snippets/assets/domain/00_HOME/Eric%20Shreck%20Gif.gif'
---

import { PortalHeroContent, HeroImageBackgroundComponent, LogoHeroContainer, HeroContentContainer, HeroSectionContainer, PortalCardsHeader, PortalContentContainer } from '/snippets/components/scaffolding/portals/Portals.jsx'
import { Starfield } from "/snippets/components/scaffolding/heroes/StarfieldCanvas.jsx";
import { H1, H2, P } from '/snippets/components/scaffolding/frame-mode/FrameMode.jsx'
import { BlinkingIcon } from '/snippets/components/elements/icons/Icons.jsx'
import { LivepeerIcon } from '/snippets/components/elements/icons/Icons.jsx'
import { CustomDivider } from '/snippets/components/elements/spacing/Divider.jsx'


<HeroSectionContainer>

 {/* HeroImageBackgroundComponent: Full-width Starfield Background - fills entire content area */}
  <HeroImageBackgroundComponent>
    <Starfield density={4.0} />
  </HeroImageBackgroundComponent>
  <HeroContentContainer>
    <LogoHeroContainer imgHeight="70px">
 DOCS
    </LogoHeroContainer>
    <PortalHeroContent
      title=" "
      subtitle="Open AI-Infrastructure for Real-Time Interactive Video"
      description="Livepeer powers the future of creative media with a full-stack on-demand AI-infrastructure for real-time video streaming & AI workflows, optimised for performance, designed for innovators."
      divider={false}
    >
      <Frame caption="Experience Interactive Real-time AI Video with Livepeer">
        <img
          src="https://raw.githubusercontent.com/livepeer/docs/docs-v2-assets/snippets/assets/domain/00_HOME/Eric%20Shreck%20Gif.gif"
          alt="Daydream Real-time AI example gif shreck interactive AI video"
        />
      </Frame>
      <br/>
      <br/>
      <Subtitle style={{fontSize: "1.2rem", fontWeight: "500"}}>
 From interactive livestreaming to generative world models, build & deploy real-time applications, workflows and models on Livepeer's open-access, low-latency network.
      </Subtitle>
    </PortalHeroContent>

    </HeroContentContainer>
</HeroSectionContainer>

{/* <p style={{ fontSize: "1.5rem", color: "var(--accent)", fontWeight: "500", opacity: 1, fontStyle: "italic" }}> */}
{/* Interactive Real-time Video Re-Imagined by Open AI-Infrastructure */}
{/* </p>
<p style={{ fontSize: "1.5rem", color: "var(--accent)", fontWeight: "500", opacity: 1, fontStyle: "italic" }}> */}
 {/* The Real-time AI Layer Redefining Interactive Video Experiences

Open AI-Infrastructure powering re-imagining Real-time Video. */}
{/* Experience The Open AI-Infrastructure Redefining Interactive Video */}
{/* </p> */}
 {/* {<div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem" }}>
      <p style={{ fontSize: "1.5rem", color: "var(--accent)", fontWeight: "500", opacity: 1 }}>
 Open AI-Infrastructure for Real-Time Interactive Video
      </p>
 </div>} */}

{/* <PageHeader
  title=" "
      subtitle="Open AI-Infrastructure for Real-Time Interactive Video"
      description="Livepeer powers the future of creative media with a full-stack on-demand AI-infrastructure for real-time video streaming & AI workflows, optimised for performance, designed for innovators."
/> */}

<PortalContentContainer>

  <PortalCardsHeader title="Livepeer Mission Control">
    <BlinkingIcon icon="circle-play" size={20}  />
  </PortalCardsHeader>

  <Columns cols={3}>
    <Card
      title="Learn About Livepeer"
      href="/v2/about/concepts/livepeer-overview"
      arrow="true"
      img="/snippets/assets/media/heros/home/hero_about.png"
    >
 {/* About the Livepeer Protocol */}
    </Card>
    <Card
      title="Stream Video In Real-Time"
      href="/v2/developers/get-started/transcoding-quickstart"
      arrow="true"
      img="/snippets/assets/media/heros/home/hero_video_stream.png"
    >
 {/* Broadcast Real-Time Video */}
    </Card>
    <Card
      title="Run AI Pipelines"
      href="/v2/developers/get-started/ai-quickstart"
      arrow="true"
      img="/snippets/assets/media/heros/home/hero_ai_run.png"
    >
 {/* Run Real-time AI & AI Pipelines on Livepeer */}
    </Card>
    <Card
      title="Earn From Your GPU"
      href="/v2/orchestrators/quickstart/guide"
      arrow="true"
      img="/snippets/assets/media/heros/home/hero_gpu.png"
    >
 {/* Add Your GPU or Data Centre To The Livepeer Network */}
    </Card>
    <Card
      title="Connect With Community"
      href="/v2/community/portal"
      arrow="true"
      img="/snippets/assets/media/heros/home/hero_community.png"
    >
 {/* Connect With The Vibrant Livepeer Community */}
    </Card>
    <Card
      title="Explore Livepeer Projects"
      href="/v2/home/solutions/showcase"
      arrow="true"
      img="/snippets/assets/media/heros/home/hero_showcase.png"
    >
 {/* Be Inspired By The Livepeer Project Showcase */}
    </Card>
    <Card
      title="Run A Livepeer Gateway Application"
      href="/v2/gateways/setup/guide"
      arrow="true"
      img="/snippets/assets/media/heros/home/hero_gateways.png"
    >
 {/* Access Compute or Set up a Gateway on Livepeer */}
    </Card>
    <Card
      title="Learn About Delegators"
      href="/v2/delegators/concepts/overview"
      arrow="true"
      img="/snippets/assets/media/heros/home/hero_delegators.png"
    >
 {/* Learn About The Livepeer Token & How to Delegate LPT */}
    </Card>
      <Card
      title="Explore Livepeer Opportunities"
      href="/v2/developers/guides/opportunities/overview"
      arrow="true"
      img="/snippets/assets/media/heros/home/hero_opportunity.png"
    >
 {/* Explore Builder Opportunities at Livepeer */}
    </Card>
  </Columns>

</PortalContentContainer>
