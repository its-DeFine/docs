---
title: Exigences de configuration en chaîne
description: >-
  Cette page vous guidera à travers la configuration des exigences
  supplémentaires pour exécuter un Gateway en mode On-Chain, y compris la
  recherche d'une URL RPC, ainsi que la création d'un compte Ethereum et d'un
  keystore.
sidebarTitle: Configuration en chaîne
icon: chain
keywords:
  - livepeer
  - gateways
...
---
{/* codex-i18n:  */}
...
<Columns cols={3}>
    ...
    <div>
        Related Pages
        - [Run a Gateway](/v2/fr/gateways/run-a-gateway/quickstart/quickstart-a-gateway)
...
Livepeer est actuellement déployé sur le [Arbitrum](https://docs.arbitrum.io/build-decentralized-apps/public-chains#arbitrum-one) réseau L2 (Arbitrum One).

Pour interagir avec le [Livepeer contrats intelligents](../../../../resources/references/contract-addresses) vous devrez vous connecter à un Arbitrum RPC.

Si vous prévoyez d'exécuter votre Gateway dans **Sur la chaîne** mode, vous aurez besoin de :
...
#### Drapeaux On-Chain requis
    ...
    <Icon icon="wifi" /> _Configuration du réseau_
    ...
    <Icon icon="coins" /> _Configuration du compte_
...
## <Icon icon="fast-forward" size={24} /> Configuration facile avec Livepeer outils
...
>
  ...
  -> then come back to this guide.{' '}
    ...
    Livepeer créera automatiquement un nouveau portefeuille et un keystore pour vous si vous ne fournissez pas d'adresse de portefeuille existante après la première[installation de votre logiciel Gateway](/v2/gateways/run-a-gateway/install/install-overview).

    Le logiciel détectera qu'aucun keystore n'existe pour cette adresse et vous demandera de:
    ...
    Utilisez la commande ci‑dessous pour démarrer rapidement une passerelle on‑chain avec un nouveau portefeuille et générer automatiquement tous les paramètres requis.

    Voir [RPC URL](#rpc-url) pour des instructions sur la recherche ou l’utilisation d’une alternative Arbitrum RPC URL.
...
Les données de compte sont stockées dans le `~/.lpData` répertoire par défaut.

Les fichiers de keystore sont créés par défaut dans :
