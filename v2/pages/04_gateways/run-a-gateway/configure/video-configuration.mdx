...
icon: 'film-canister'
keywords: ["livepeer", "gateways", "run a gateway", "configure", "video configuration", "video", "configuration", "services"]
...
livepeer -gateway \
  ...
  # You can also use a JSON file: -orchAddr=/path/to/05_orchestrators/orchestrators-portal.json
```
</Tab>
...
Add these flags for on-chain configuration. See <DoubleIconLink label="On-Chain Setup Guide" href="/v2/pages/04_gateways/run-a-gateway/requirements/on-chain%20setup/on-chain" iconLeft="link" /> for details.
