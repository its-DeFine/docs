...
- $b_{D,O}$: stake bonded by $D$ toward $O$
- $B_{self,O}$: self-bonded stake of $O$
...
Delegators selecting $O$ increase $W_O$, affecting issuance allocation and governance influence.
...
$$
R_{D,O} = R_O \cdot (1 - c_O) \cdot \frac{b_{D,O}}{B_O}
$$

Delegator total return decomposes into:

$$
\text{Reward}_{D,O} = \text{Issuance}_{D,O} + \text{Fees}_{D,O}
$$
...
$$
U(O) = f(\text{NetYield}_O, \text{Reliability}_O, \text{Concentration}_O, \text{GovernanceAlignment}_O)
$$

Where:

- $\text{NetYield}_O$ is reduced by commission $c_O$
- $\text{Reliability}_O$ captures checkpoint consistency and operational stability
- $\text{Concentration}_O$ penalizes already-dominant stake share
- $\text{GovernanceAlignment}_O$ reflects long-term stewardship preferences
