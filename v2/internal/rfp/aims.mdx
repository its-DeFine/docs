...
sidebarTitle: Aims
description: >-
  This page articulates the philosophical framework that underpinned the v2 engagement and demonstrates how the RFP's
  three overarching themes for **stakeholder-focused**, **AI-first**, and **future-proofed** documentation, were
  delivered across every layer of the system.
...
<Tabs>
 ...
 The contribution model was designed from the outset to support open-source contribution via GitHub PRs, beyond internal authorship. Issue templates, PR templates, CODEOWNERS, and a Discord issue intake workflow make external contribution low-friction. The governance.mdx and source-of-truth-policy.md documents define canonical ownership boundaries so contributors know exactly where content authority lives.
        ...
        - **[Internal docs]** docs-guide/ internal operating system: overview.mdx (source-of-truth model, update rules), features/feature-map.mdx (repo feature areas), features/architecture-map.mdx (data/control flow with Mermaid diagram), tooling/lpd-cli.mdx (CLI operator runbooks), policies/quality-gates.mdx (local/CI enforcement matrix), features/automations.mdx (automation surfaces), frameworks/content-system.mdx (IA model, copy principles), features/data-integrations.mdx (API specs, external feeds). Three generated catalogs: scripts-catalog.mdx, workflows-catalog.mdx, templates-catalog.mdx - all auto-updated when contents change.
