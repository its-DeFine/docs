...
description: >-
  The RFP identified four specific problems with the existing Livepeer
  documentation. This section documents how v2 addressed each one: the
  decision-making process, the pre-work required, the options considered, and
  the full implementation evidence - tagged by delivery layer.
Keywords:
  - livepeer
  - internal
  - rfp
...
<AccordionGroup>
    ...
    <Accordion title="Implementation">
        - **[IA]** Nine top-level sections in docs.json, each a direct stakeholder entry point: Home (orientation), About (network understanding), Platforms (product landscape), Developers (builders), Gateways (gateway operators), GPU Nodes (orchestrators), Delegators, Community (ecosystem), Resource HUB (references). No toggle required.
...
<AccordionGroup>
    ...
    <Accordion title ="Constraints">
        Full verified integration with the governance portal has not been audited as a discrete deliverable. The governance portal is linked from the Delegators section, but whether the bidirectional connection (docs linked from governance portal) has been established is not confirmed. This is listed as an open item in the close-out task list.
