...
description: >-
  Complete guide to all automation scripts, GitHub Actions workflows, n8n automations, and pre-commit hooks in the
...
2. Runs `.github/scripts/update-livepeer-release.js` to resolve and write the canonical release data
3. Commits the canonical data file if a new version is available
...
3. Update `snippets/assets/data/n8n/Discord-Issue-Intake.json` to emit the new version and fields
...
- `snippets/_workspace/archive/assets-scripts/README-discord-issue-intake-workflow.md`
...
2. Updates `snippets/data/social-feeds/forumData.jsx`
...
2. Updates `snippets/data/social-feeds/ghostBlogData.jsx`
3. Commits and pushes if changes detected
...
3. Updates `snippets/data/social-feeds/youtubeData.jsx`
...
n8n workflows are JSON files located in `snippets/assets/data/n8n/`. These workflows run on an external n8n instance and can be imported/configured there. Superseded repo-tracked definitions now also have tracked archive copies under `tasks/staging/deprecated-n8n/`.
...
**Output:** `snippets/data/social-feeds/discordAnnouncementsData.jsx`
...
**Output:** `snippets/data/social-feeds/lumaEventsData.jsx`
...
- **Ghost-to-Mintlify.json** - archived in favour of `update-ghost-blog-data.yml`
- **Forum-To-Mintlify-Latest-Topics.json** - archived in favour of `update-forum-data.yml`
- **YouTube-To-Mintlify.json** - archived in favour of `update-youtube-data.yml`
...
Scripts are organised into multiple directories based on their purpose. All scripts use git-based repo root detection with fallback to `paths.config.json`.
...
node operations/scripts/snippets/generate-og-images.js

# Dry run the frontmatter rewrite
node operations/scripts/snippets/generate-seo.js --dry-run

# Process all authored MDX files
node operations/scripts/snippets/generate-seo.js

# Process single file
node operations/scripts/snippets/generate-seo.js --file=v2/home/mission-control.mdx
```
...
**Usage:**
```bash icon="terminal"
./operations/scripts/snippets/generate-api-docs.sh <openapi-spec> <output-dir> <api-name> [github-repo-url]
```
...
**What it does:**
1. Reads `docs-guide/config/component-registry.json`
...
3. Saves to `snippets/composables/pages/shared/`

**Downloads to `snippets/composables/pages/shared/`:**
...
**Usage:**
```bash icon="terminal"
./operations/scripts/snippets/fetch-lpt-exchanges.sh
```
...
4. Updates `v2/delegators/resources/compendium/exchanges.mdx`
...
# or
node operations/scripts/test-v2-pages.js
```
...
- [Style Guide](../copy-style/style-guide) - Styling conventions and Mintlify gotchas
...
- [Scripts README](/operations/scripts/README.md) - Detailed scripts documentation
