...
testFiles.forEach(testFile => {
  ...
  const newFrontmatter = rebuildFrontmatter(originalFields, newKeywords, newOgImage);
  ...
  if (fieldsPreserved) {
    console.log('✓ All frontmatter fields preserved (except keywords and og:image)');
  }
  
  // Show what changed
  console.log('\nChanges:');
  console.log(`  Keywords: ${JSON.stringify(result.oldKeywords)} → ${JSON.stringify(result.newKeywords)}`);
  ...
  for (const key in originalFields) {
    if (key !== 'keywords' && key !== 'og:image' && key !== 'og') {
      const value = originalFields[key].length > 50 
...
if (allTestsPassed) {
  ...
  console.log('  - Preserves all frontmatter fields except keywords and og:image');
