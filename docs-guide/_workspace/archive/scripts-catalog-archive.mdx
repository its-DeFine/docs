---
title: 'Scripts Catalog Archive'
sidebarTitle: 'Scripts Archive'
description: 'Archived snapshot of the repository scripts catalog retained for migration traceability.'
keywords:
  ['livepeer', 'scripts catalog archive', 'aggregate inventory', 'repository', 'scripts']
---
...
| Script | Summary | Usage | Owner |
|---|---|---|---|
| `operations/tests/integration/browser.test.js` | Utility script for operations/tests/integration/browser.test.js. | `node operations/tests/integration/browser.test.js` | docs |
| `operations/tests/integration/domain-pages-audit.js` | Audit deployed docs page load status and emit a stable JSON report. | `node operations/tests/integration/domain-pages-audit.js --version both` | docs |
| `operations/tests/integration/v2-link-audit.js` | Comprehensive V2 MDX link audit with internal strict checks and optional external URL validation. | `node operations/tests/integration/v2-link-audit.js --full --write-links --strict` | docs |
| `operations/tests/integration/v2-link-audit.selftest.js` | Script-level self-tests for v2 link audit external validation using a local HTTP fixture and temporary MDX file. | `node operations/tests/integration/v2-link-audit.selftest.js` | docs |
| `operations/tests/integration/v2-wcag-audit.js` | Audit v2 docs.json navigation pages for accessibility (WCAG 2.2 AA) with deterministic reports and conservative source autofixes. | `node operations/tests/integration/v2-wcag-audit.js --full` | docs |
| `operations/tests/integration/v2-wcag-audit.selftest.js` | Script-level self-tests for the v2 WCAG audit (local HTTP + Puppeteer axe run, and temp-file fix/stage behavior without Mintlify). | `node operations/tests/integration/v2-wcag-audit.selftest.js` | docs |
| `tests/run-all.js` | Utility script for tests/run-all.js. | `node operations/tests/run-all.js` | docs |
| `tests/run-pr-checks.js` | Run changed-file scoped validation checks for pull request CI. | `node operations/tests/run-pr-checks.js --base-ref main` | docs |
| `operations/tests/unit/docs-guide-sot.test.js` | Validate docs-guide source-of-truth coverage, README pointers, and generated index freshness. | `node operations/tests/unit/docs-guide-sot.test.js` | docs |
| `operations/tests/unit/docs-navigation.test.js` | Validate docs.json page-entry syntax, report missing routes, suggest remaps, and optionally apply approved remaps. | `./lpd tests unit docs-navigation.test` | docs |
| `operations/tests/unit/links-imports.test.js` | Utility script for operations/tests/unit/links-imports.test.js. | `node operations/tests/unit/links-imports.test.js` | docs |
| `operations/tests/unit/mdx-guards.test.js` | Enforce MDX guardrails for globals imports, math delimiters, and markdown table line breaks. | `node operations/tests/unit/mdx-guards.test.js` | docs |
| `operations/tests/unit/mdx.test.js` | Utility script for operations/tests/unit/mdx.test.js. | `node operations/tests/unit/mdx.test.js` | docs |
| `operations/tests/unit/quality.test.js` | Utility script for operations/tests/unit/quality.test.js. | `node operations/tests/unit/quality.test.js` | docs |
| `operations/tests/unit/script-docs.test.js` | Enforce script header schema, keep group script indexes in sync, and build aggregate script index. | `node operations/tests/unit/script-docs.test.js --staged --write --stage --autofill` | docs |
| `operations/tests/unit/spelling.test.js` | Utility script for operations/tests/unit/spelling.test.js. | `node operations/tests/unit/spelling.test.js` | docs |
| `operations/tests/unit/style-guide.test.js` | Utility script for operations/tests/unit/style-guide.test.js. | `node operations/tests/unit/style-guide.test.js` | docs |
| `operations/tests/unit/v2-link-audit.test.js` | Unit tests for v2 link audit args, external validation helpers, and x-* scope exclusion behavior. | `node operations/tests/unit/v2-link-audit.test.js` | docs |
| `operations/tests/unit/v2-wcag-audit.test.js` | Unit tests for v2 WCAG audit helper logic (args, thresholds, route mapping, report sorting, and conservative autofixes). | `node operations/tests/unit/v2-wcag-audit.test.js` | docs |
| `operations/tests/utils/file-walker.js` | Utility script for operations/tests/utils/file-walker.js. | `node operations/tests/utils/file-walker.js` | docs |
| `operations/tests/utils/mdx-parser.js` | Utility script for operations/tests/utils/mdx-parser.js. | `node operations/tests/utils/mdx-parser.js` | docs |
| `operations/tests/utils/spell-checker.js` | Utility script for operations/tests/utils/spell-checker.js. | `node operations/tests/utils/spell-checker.js` | docs |
...
| `tools/scripts/new-script.js` | Create a new script file prefilled with the required docs header template. | `node tools/scripts/new-script.js --path operations/tests/integration/my-script.js` | docs |
