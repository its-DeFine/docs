...
| `legacy_bridge_inventory` | Declares the remaining live consumers blocking retirement of `tools/config/runtime/**` bridge manifests |
...
**Purpose:** Canonical ownerless-governance surface registry during staged migration to `operations/governance/**`.

**Bridge source:** `tools/config/runtime/ownerless-governance-surfaces.json`
...
Under the staged bridge model it contains a mix of:
