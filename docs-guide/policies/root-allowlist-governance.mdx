...
- Canonical contract: `tools/config/runtime/root-governance.json`
...
Human-authored root decisions belong in `tools/config/runtime/root-governance.json`. `.allowlist` is a generated projection.
...
The validator `operations/scripts/validators/governance/compliance/check-root-governance-sync.js` must fail when those outputs drift from `tools/config/runtime/root-governance.json`.
...
1. Edit `tools/config/runtime/root-governance.json`.
