...
Legacy runtime mirrors under `tools/config/runtime/**` are retired. Active governance reads must target `operations/governance/config/**` only.
...
- `operations/tests/unit/ai-tools-registry.test.js` validates the migrating AI-tools registry surface declared in the ownerless manifest.
