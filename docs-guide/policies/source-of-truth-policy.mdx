...
- Hook-managed and CI-managed generated outputs must be declared in `tools/config/runtime/generated-artifacts.json`.
...
4. Codex skill installs must be generated from templates using `node operations/scripts/dispatch/ai/agents/sync-codex-skills.js` or the `bash lpd setup --yes` bootstrap path (manual edits in local skill folders are non-canonical).
