...
The canonical configuration lives in `tools/config/runtime/generated-artifacts.json`. Hooks and supporting utilities must read from that manifest instead of embedding path-specific generated-file rules directly in shell logic.
...
3. Hooks must act only on artifacts listed in `tools/config/runtime/generated-artifacts.json`.
