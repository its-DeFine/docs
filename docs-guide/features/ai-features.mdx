...
Mintlify auto-generates `llms.txt` and `llms-full.txt` for all hosted sites. These provide clean Markdown for LLM consumption. No repo-level configuration needed.
...
Translation pipeline for multi-locale content. Frontmatter taxonomy validator at `tools/lib/frontmatter-taxonomy.js` enforces valid locale values.
