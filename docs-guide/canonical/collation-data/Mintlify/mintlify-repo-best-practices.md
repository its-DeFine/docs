...
2. Enforcement surfaces: `operations/tests/**`, `operations/scripts/validators/**`, `operations/scripts/config/docs-path-sync.js`.
...
5. Historical research and legacy notes: `workspace/thread-outputs/research/**`, `snippets/snippetsWiki/mintlify-behaviour.mdx`, archives, translations, and deprecated docs.
...
| Mintlify behaviour references | This file is the operational canonical source. | Treating `snippets/snippetsWiki/mintlify-behaviour.mdx` or thread research notes as the first-read rulebook. |
...
- `workspace/thread-outputs/research/mintlify-constraints-reference.md` remains the best supporting research packet and evidence snapshot, but it is a research output, not the stable repo contract.
- `snippets/snippetsWiki/mintlify-behaviour.mdx` contains useful discovery history and examples, but some guidance is hypothesis-led, overly broad, or superseded by later repo evidence.
