...
The governed script library currently indexes **345** script(s).
...
| `generate-snippets-registry.js` | Generates the snippets root registry from the live snippets filesystem plus guide-owned folder metadata. | P3 | governance | `node operations/scripts/generators/governance/catalogs/generate-snippets-registry.js [--write&#124;--check]` |
...
| `snippets-root-governance.test.js` | Validates the two-file snippets root governance model and enforces freshness for the generated snippets registry. | P3 | governance | `node operations/tests/unit/snippets-root-governance.test.js` |
...
| `update-livepeer-release.test.js` | Verifies the go-livepeer release updater writes the canonical release data module shape. | P2 | maintenance | `node operations/tests/unit/update-livepeer-release.test.js` |
...
<Accordion title="⚠️ Unclassified — 102 scripts need @type">
