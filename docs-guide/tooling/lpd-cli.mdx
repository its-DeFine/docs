...
purpose: reference
status: current
lastVerified: 2026-03-27T00:00:00.000Z
...
<Note>
`lpd dev` defaults to port `3333` when no port is provided and rejects port `3000`.
</Note>
...
```bash icon="terminal"
bash tools/dev/mint-custom-loader.sh docs-orch-work.json
```
...
<AccordionGroup>
  ...
  <ResponseField name="lpd dev" type="command">
    ...
    **Flags**
    ...
    <ResponseField name="--script [path]" type="string" default="tools/dev/mint-dev.sh">
  ...
  <ResponseField name="mint-custom-loader helper" type="command">
    ...
    **Usage**
    ```bash icon="terminal"
    bash tools/dev/mint-custom-loader.sh <custom-docs-json> [-- ...mint args]
    ```

    **Examples**
    ```bash icon="terminal"
    bash tools/dev/mint-custom-loader.sh docs-orch-work.json
    bash tools/dev/mint-custom-loader.sh docs-gate-work.json -- --port 3333
    ```
  ...
  <ResponseField name="Slow Mint startup on large navs" type="troubleshooting">
    ...
    ```bash icon="terminal"
    bash tools/dev/mint-custom-loader.sh docs-orch-work.json
    bash tools/dev/mint-custom-loader.sh docs-gate-work.json
    ```
  </ResponseField>
...
<TabsContainer>
  ...
  |---|---|---|---|
  | `audit-all-pages-simple` | script-specific via `-- <args>` | `operations/scripts/audit-all-pages-simple.js` | Utility script for tasks/scripts/audit-all-pages-simple.js. |
  | `audit-all-pages` | script-specific via `-- <args>` | `operations/scripts/audit-all-pages.js` | Utility script for tasks/scripts/audit-all-pages.js. |
  | `audit-all-v2-pages` | script-specific via `-- <args>` | `operations/scripts/audit-all-v2-pages.js` | Utility script for operations/scripts/audit-all-v2-pages.js. |
  | `audit-scripts` | script-specific via `-- <args>` | `operations/scripts/audit-scripts.js` | Audit full-repo executable scripts, categorize usage/overlap, and overwrite SCRIPT_AUDIT reports. |
  | `audit-tasks-folders` | script-specific via `-- <args>` | `operations/scripts/audit-tasks-folders.js` | Audit tasks folders, optionally normalise report locations, and optionally apply audit recommendations with conflict-safe moves. |
  | `audit-v2-usefulness` | script-specific via `-- <args>` | `operations/scripts/audit-v2-usefulness.js` | Audit v2 MDX pages (excluding x-* directories) and emit page-level usefulness matrix rows with source-weighted 2026 accuracy verification fields. |
  | `convert-rss-to-mdx` | script-specific via `-- <args>` | `operations/scripts/convert-rss-to-mdx.js` | Runs `operations/scripts/convert-rss-to-mdx.js`. |
  | `debug-mint-dev` | script-specific via `-- <args>` | `operations/scripts/debug-mint-dev.js` | Utility script for operations/scripts/debug-mint-dev.js. |
  ...
  | `dev add-callouts` | script-specific via `-- <args>` | `tools/dev/add-callouts.js` | Utility script for tools/dev/add-callouts.js. |
  ...
  | `dev ensure-mint-watcher-patch` | script-specific via `-- <args>` | `tools/dev/ensure-mint-watcher-patch.sh` | Ensure Mint local-preview watcher disables glob expansion in repo paths. |
  ...
  | `dev test-add-callouts` | script-specific via `-- <args>` | `tools/dev/test-add-callouts.js` | Utility script for tools/dev/test-add-callouts.js. |
  | `generate-ai-sitemap` | script-specific via `-- <args>` | `operations/scripts/generate-ai-sitemap.js` | Generate an AI-focused sitemap from v2 docs navigation. |
  | `generate-docs-guide-indexes` | script-specific via `-- <args>` | `operations/scripts/generate-docs-guide-indexes.js` | Generate docs-guide workflow/template indexes and optionally verify they are up to date. |
  | `generate-docs-guide-pages-index` | script-specific via `-- <args>` | `operations/scripts/generate-docs-guide-pages-index.js` | Generate docs-guide/catalog/pages-catalog.mdx from v2/index.mdx entries filtered to docs.json navigation pages. |
  | `generate-docs-index` | script-specific via `-- <args>` | `operations/scripts/generate-docs-index.js` | Generate docs-index.json and optionally backfill v2 frontmatter metadata. |
  | `generate-llms-files` | script-specific via `-- <args>` | `operations/scripts/generate-llms-files.js` | Generate llms.txt and llms-full.txt from v2 docs navigation. |
  | `generate-pages-index` | script-specific via `-- <args>` | `operations/scripts/generate-pages-index.js` | Generate and verify section-style index.mdx files for v2 docs folders, plus the root aggregate index. |
  | `mint-dev` | script-specific via `-- <args>` | `operations/scripts/mint-dev.sh` | Utility script for operations/scripts/mint-dev.sh. |
  | `new-script` | script-specific via `-- <args>` | `operations/scripts/new-script.js` | Create a new script file prefilled with the required docs header template. |
  | `publish-v2-internal-reports` | script-specific via `-- <args>` | `operations/scripts/publish-v2-internal-reports.js` | Duplicate approved markdown reports into v2/internal/reports pages with metadata and update docs.json. |
  | `snippets fetch-external-docs` | script-specific via `-- <args>` | `operations/scripts/snippets/fetch-external-docs.sh` | Utility script for operations/scripts/snippets/fetch-external-docs.sh. |
  ...
  | `generate-component-docs` | `--dry-run`, `--fix`, `--check`, `--template-only`, `--category <name>` | `operations/scripts/generate-component-docs.js` | Generates the published component-library pages and localized scaffolds from governed component metadata. |
  | `test-all-pages-browser` | script-specific via `-- <args>` | `operations/scripts/test-all-pages-browser.js` | Utility script for operations/scripts/test-all-pages-browser.js. |
  | `test-all-pages-comprehensive` | script-specific via `-- <args>` | `operations/scripts/test-all-pages-comprehensive.js` | Utility script for operations/scripts/test-all-pages-comprehensive.js. |
  | `test-v2-pages` | script-specific via `-- <args>` | `operations/scripts/test-v2-pages.js` | Utility script for operations/scripts/test-v2-pages.js. |
  | `test-youtube-pages` | script-specific via `-- <args>` | `operations/scripts/test-youtube-pages.js` | Utility script for operations/scripts/test-youtube-pages.js. |
  ...
  | `wcag-repair-common` | script-specific via `-- <args>` | `operations/scripts/wcag-repair-common.js` | Apply conservative WCAG-related source autofixes across v2 docs (common raw-tag issues) and write deterministic repair reports. |
  </Tab>
  <Tab title="Tasks">
  ...
  | `integration v2-link-audit` | script-specific via `-- <args>` | `operations/tests/integration/v2-link-audit.js` | Comprehensive V2 MDX link audit with internal strict checks and optional external URL validation. |
  ...
  | `unit links-imports.test` | script-specific via `-- <args>` | `operations/tests/unit/links-imports.test.js` | Utility script for operations/tests/unit/links-imports.test.js. |
  ...
  | `unit v2-link-audit.test` | script-specific via `-- <args>` | `operations/tests/unit/v2-link-audit.test.js` | Unit tests for v2 link audit args, external validation helpers, and x-* scope exclusion behaviour. |
  ...
  | Script token | Flags available | Resolved path | PURPOSE |
  |---|---|---|---|
  | `add-callouts` | script-specific via `-- <args>` | `tools/dev/add-callouts.js` | Utility script for tools/dev/add-callouts.js. |
  ...
  | `ensure-mint-watcher-patch` | script-specific via `-- <args>` | `tools/dev/ensure-mint-watcher-patch.sh` | Ensure Mint local-preview watcher disables glob expansion in repo paths. |
  | `replace-og-image` | script-specific via `-- <args>` | `tools/dev/replace-og-image.py` | Utility script for tools/dev/replace-og-image.py. |
  | `test-add-callouts` | script-specific via `-- <args>` | `tools/dev/test-add-callouts.js` | Utility script for tools/dev/test-add-callouts.js. |
