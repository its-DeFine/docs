...
## Agent Adapter Surface Map

All active native adapter files and their owning tools. Each adapter should be a thin pointer back to `AGENTS.md`; policy must not drift between surfaces.

| Agent family | Canonical adapter path | Notes |
|---|---|---|
| Cross-agent baseline | `AGENTS.md` | Primary source of truth for all repo-wide rules |
| GitHub Copilot | `.github/copilot-instructions.md` | GitHub-native path |
| Claude Code | `.claude/CLAUDE.md` | Claude Code native path |
| Cursor (modern) | `.cursor/rules/repo-governance.mdc` + `.cursor/rules/no-deletions.mdc` | Active; replaces `.cursorrules` |
| Cursor (legacy) | `.cursorrules` | Backward-compat stub pointing to `.cursor/rules/` |
| Windsurf | `.windsurf/rules/repo-governance.md` | Windsurf native path |
| Augment Code | `.augment/rules/repo-governance.md` + `.augment/rules/git-safety.md` + `.augment/rules/no-deletions.md` | Added Phase 9 |
| Mintlify Assistant | `.mintlify/Assistant.md` | Chat widget context for the docs site |
| Codex (task layer) | `.github/AGENTS.md` | Codex-specific HitL + checkpoint rules on top of root `AGENTS.md` |
...
Shared local-preview rule across all active agent-governance surfaces:
...
```bash icon="terminal"
node operations/scripts/validators/content/veracity/docs-fact-registry.js --validate --registry tasks/research/claims
...
node operations/scripts/dispatch/content/veracity/docs-research-packet.js --nav tools/config/scoped-navigation/docs-gate-work.json --version v2 --language en --tab Orchestrators --group Guides --out tasks/reports/orchestrator-guides-review/research-guides-review
```
