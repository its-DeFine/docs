...
The current operational root remains `ai-tools/`, but the governed model is now wider than the older `templates + local skills + exports` framing.

Current implementation rule:

- keep the active root and existing entrypoints working
- consolidate logic toward one canonical source
- keep agent-specific wrappers thin
- treat generated outputs as derived only
...
This is a target model for documentation, classification, and migration. It is not a mandate to move every file immediately.
...
| Cursor (modern) | `.cursor/rules/*.mdc` | `repo-governance.mdc`, `no-deletions.mdc` | Active; replaces legacy `.cursorrules` |
| Cursor (legacy) | `.cursorrules` | `.cursorrules` | Backward-compat stub pointing to `.cursor/rules/` |
...
Legacy stubs retained on disk: `ASSISTANT.md` (redirect stub pointing to `.mintlify/Assistant.md`).
