...
| --- | --- | --- | --- | --- |
| `data-refresh` | data-refresh | `ubuntu-latest` | none | 8 |
...
- `Resolve livepeer release version` | runs `if [[ -n "${{ inputs.version }}" ]]; then`
- `Read current release version` | runs `CURRENT=$(grep -oP 'latestVersion = "\K[^"]+' snippets/automations/globals/globals.jsx || echo "")`
