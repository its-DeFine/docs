...
source_path: '.github/workflows/generate-ai-sitemap.yml'
...
Govern the `.github/workflows/generate-ai-sitemap.yml` workflow as a human-readable, visually explorable source-of-truth page inside `ai-tools/registry/workflows`.
...
| --- | --- | --- | --- | --- |
| `generate-ai-sitemap` | generate-ai-sitemap | `ubuntu-latest` | none | 7 |
...
- `Generate AI sitemap` | runs `node operations/scripts/generate-ai-sitemap.js --write`
- `Check for changes` | runs `git diff --exit-code sitemap-ai.xml || echo "changed=true" >> $GITHUB_OUTPUT`
...
flowchart TD
  ...
  workflow["Workflow: Generate AI Sitemap"]
  trigger --> workflow
  job1["Job: generate-ai-sitemap"]
  workflow --> job1
