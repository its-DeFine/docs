...
source_path: '.github/workflows/verify-llms-files.yml'
...
Govern the `.github/workflows/verify-llms-files.yml` workflow as a human-readable, visually explorable source-of-truth page inside `ai-tools/registry/workflows`.
...
| --- | --- | --- | --- | --- |
| `verify-llms-files` | verify-llms-files | `ubuntu-latest` | none | 4 |
...
- `Verify llms outputs` | runs `node operations/scripts/generate-llms-files.js --check`
...
flowchart TD
  ...
  workflow["Workflow: Verify llms.txt Files"]
  trigger --> workflow
  job1["Job: verify-llms-files"]
  workflow --> job1
