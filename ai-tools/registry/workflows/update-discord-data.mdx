...
status: 'active'
concern: 'authoring'
source_path: '.github/workflows/update-discord-data.yml'
...
# Update Discord Announcements Data
...
Govern the `.github/workflows/update-discord-data.yml` workflow as a human-readable, visually explorable source-of-truth page inside `ai-tools/registry/workflows`.
...
| --- | --- | --- | --- | --- |
| `update-discord-data` | update-discord-data | `ubuntu-latest` | none | 5 |

### update-discord-data

- `Checkout repository` | uses actions/checkout@v4
- `Setup Node.js` | uses actions/setup-node@v4
- `Fetch Discord announcements` | runs `node .github/scripts/fetch-discord-announcements.js`
...
flowchart TD
  ...
  workflow["Workflow: Update Discord Announcements Data"]
  trigger --> workflow
  job1["Job: update-discord-data"]
  workflow --> job1
