...
status: 'active'
concern: 'authoring'
source_path: '.github/workflows/update-rss-blog-data.yml'
...
Govern the `.github/workflows/update-rss-blog-data.yml` workflow as a human-readable, visually explorable source-of-truth page inside `ai-tools/registry/workflows`.
...
| --- | --- | --- | --- | --- |
| `update-rss-data` | update-rss-data | `ubuntu-latest` | none | 5 |

### update-rss-data

- `Checkout repository` | uses actions/checkout@v4
- `Setup Node.js` | uses actions/setup-node@v4
- `Fetch RSS blog data` | runs `node .github/scripts/fetch-rss-blog-data.js`
...
flowchart TD
  ...
  workflow["Workflow: Update RSS Blog Data"]
  trigger --> workflow
  job1["Job: update-rss-data"]
  workflow --> job1
