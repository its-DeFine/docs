...
export const ContractAddressDisplay = ({
    ...
    <span style={{ display: "inline-flex", alignItems: "center", gap: "0.25rem" }}>
      ...
      <button
        ...
        style={{
          ...
          padding: "2px 4px",
          fontSize: "0.75rem",
          color: copiedAddress === address ? "var(--primary)" : "var(--text-secondary)",
  ...
  const filteredArb = arbEntries.filter((e) => {
    ...
    const q = !arbQuery || e.name.toLowerCase().includes(arbQuery.toLowerCase()) || e.address.toLowerCase().includes(arbQuery.toLowerCase());
    return cat && q;
  });

  const filteredEth = ethEntries.filter((e) => {
    const cat = ethCategory === "All" || e.category === ethCategory;
    const q = !ethQuery || e.name.toLowerCase().includes(ethQuery.toLowerCase()) || e.address.toLowerCase().includes(ethQuery.toLowerCase());
    return cat && q;
  });

  const tableStyles = {
    wrapper: { marginBottom: "1.5rem" },
    controls: { display: "flex", flexWrap: "wrap", gap: "0.5rem", marginBottom: "0.75rem" },
    input: { flex: 1, minWidth: "200px", maxWidth: "400px", padding: "8px 12px", borderRadius: "8px", border: "1px solid var(--border)", background: "var(--background)", color: "var(--text)" },
    select: { minWidth: "150px", padding: "8px 12px", borderRadius: "8px", border: "1px solid var(--border)", background: "var(--background)", color: "var(--text)" },
    table: { width: "100%", borderCollapse: "collapse", fontSize: "0.85rem" },
    th: { textAlign: "left", padding: "0.5rem", borderBottom: "2px solid var(--border)" },
    tr: { borderBottom: "1px solid var(--border)" },
    td: { padding: "0.5rem" },
    count: { fontSize: "0.75rem", color: "var(--text-secondary)", marginTop: "0.25rem" },
  };
  ...
  return (
    <div className={className} style={style}>
      ...
      <div style={tableStyles.controls}>
        <input type="text" value={arbQuery} onChange={(e) => setArbQuery(e.target.value)} placeholder="Search by name or address..." style={tableStyles.input} />
        <select value={arbCategory} onChange={(e) => setArbCategory(e.target.value)} style={tableStyles.select}>
          <option value="All">All categories</option>
          {arbCategories.map((cat) => <option key={cat} value={cat}>{cat}</option>)}
        </select>
      </div>
          ...
          <tbody>
            {filteredArb.map((entry, i) => (
              <tr key={`arb-${entry.address}-${i}`} style={tableStyles.tr}>
                <td style={{ ...tableStyles.td, whiteSpace: "nowrap", fontWeight: 600 }}>{entry.name}{entry.version ? ` (${entry.version})` : ""}</td>
                <td style={tableStyles.td}>{addressCell(entry.address, arbscanBase, "Arbiscan")}</td>
                <td style={{ ...tableStyles.td, fontSize: "0.8rem", color: "var(--text-secondary)" }}>{entry.type}</td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
      <div style={tableStyles.count}>{filteredArb.length} of {arbEntries.length} contracts</div>

      {/* ── Ethereum Mainnet ─────────────────────── */}
      <h3 style={{ marginBottom: "0.5rem", marginTop: "1.5rem" }}>Ethereum Mainnet</h3>
      <div style={tableStyles.controls}>
        <input type="text" value={ethQuery} onChange={(e) => setEthQuery(e.target.value)} placeholder="Search by name or address..." style={tableStyles.input} />
        <select value={ethCategory} onChange={(e) => setEthCategory(e.target.value)} style={tableStyles.select}>
          <option value="All">All categories</option>
          {ethCategories.map((cat) => <option key={cat} value={cat}>{cat}</option>)}
        </select>
      </div>
      <div style={{ overflowX: "auto", marginBottom: "0.25rem" }}>
        <table style={tableStyles.table}>
          ...
          <tbody>
            {filteredEth.map((entry, i) => (
              <tr key={`eth-${entry.address}-${i}`} style={tableStyles.tr}>
                <td style={{ ...tableStyles.td, whiteSpace: "nowrap", fontWeight: 600 }}>{entry.name}{entry.version ? ` (${entry.version})` : ""}</td>
                <td style={tableStyles.td}>{addressCell(entry.address, ethscanBase, "Etherscan")}</td>
                <td style={{ ...tableStyles.td, fontSize: "0.8rem", color: "var(--text-secondary)" }}>{entry.type}</td>
              </tr>
            ))}
          </tbody>
      ...
      <div style={tableStyles.count}>{filteredEth.length} of {ethEntries.length} contracts</div>
      ...
      {showHistorical && (
        <div style={{ marginTop: "1.5rem" }}>
          ...
          {[
            { title: "Arbitrum One — Historical", historical: data.arbitrumOne.historical, explorerBase: arbscanBase, explorerLabel: "Arbiscan" },
            { title: "Ethereum Mainnet — Historical", historical: data.ethereumMainnet.historical, explorerBase: ethscanBase, explorerLabel: "Etherscan" },
            ...
            return (
              <div key={title} style={{ marginBottom: "1rem" }}>
                <details>
                  <summary style={{ cursor: "pointer", fontWeight: 600, padding: "0.5rem 0" }}>{title}</summary>
                        ...
                        <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
                          {(groupData.entries || []).map((entry) => (
                            <li key={entry.address} style={{ padding: "0.25rem 0" }}>
                              ...
                              {addressCell(entry.address, explorerBase, explorerLabel)}
                            </li>
                          ))}
                        </ul>
                      </div>
