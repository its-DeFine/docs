...
<Tip>
  ...
  <span style={{color: "var(--hero-text)", marginTop: "0.5rem"}}> **Last Verified:** <DataWrap value={contractAddresses.meta.lastVerified} /> </span>
</Tip>
...
export const activeTableItems = (() => {
  const chainLabel = { arbitrumOne: "Arbitrum One", ethereumMainnet: "Ethereum Mainnet" }
  const explorerBase = {
  ...
  return allCurrentContracts.map((c) => ({
    "Name": c.name,
    "Address": c.address,
...
<AccordionGroup>
  ...
  <Accordion title={<AccordionTitle icon={<LivepeerIcon color="var(--accent)" size={16} />} title="Genesis" description="Historical libraries and helper contracts from the 2018 launch" />}>
    <HistoricalContractTable contracts={["MerkleProof"]} sourceData={contractAddresses} />
  </Accordion>
</AccordionGroup>
