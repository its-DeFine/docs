...
<Tip>
  ...
  Data is sourced from the <DoubleIconLink label="Livepeer GitHub" href="https://github.com/livepeer/governor-scripts/blob/master/updates/addresses.js" iconRightColor="var(--hero-text)"/> and verified on-chain.
...
This table lists only currently active standalone contract entrypoints used for standard protocol reference. Active proxy contracts are listed separately below. Paused, migration-residual, and legacy-operational contracts are grouped into a separate section so the main searchable table stays active-only.

<Accordion title="See Workflow Verification Information">
  The information in these contracts is populated by the GitHub Action [`update-contract-addresses.yml`](https://github.com/livepeer/docs/blob/main/.github/workflows/update-contract-addresses.yml).

    - Cron: Weekly Monday 02:00 UTC
    - Manual dispatch: From Actions tab with inputs (`dry_run`, `skip_verify`, `use_test_branch`)
    - Repository dispatch: `governor-scripts-update`
</Accordion>
...
export const lifecycleLabel = {
  active: "Active",
  paused: "Paused",
  migration_residual: "Migration residual",
  legacy_operational: "Legacy operational",
  historical: "Historical",
}

export const explorerBase = {
  arbitrumOne: contractAddresses.meta?.explorerUrls?.arbiscanAddress || `${contractAddresses.meta?.explorerUrls?.arbiscan || "https://arbiscan.io"}/address/`,
  ethereumMainnet: contractAddresses.meta?.explorerUrls?.etherscanAddress || `${contractAddresses.meta?.explorerUrls?.etherscan || "https://etherscan.io"}/address/`,
}

export const normalizeContractsForTable = (entries, chain, fallbackLifecycle = "active") =>
  (entries || []).map((contract) => {
    ...
    return {
      ...
      "_addressHref": contract.blockchainHref || `${explorerBase[chain]}${contract.address}`,
      "Type": type,
...
export const activeEntries = [
  ...normalizeContractsForTable(
    contractAddresses.arbitrumOne?.active || contractAddresses.arbitrumOne?.current,
    "arbitrumOne",
    "active",
  ),
  ...normalizeContractsForTable(
    contractAddresses.ethereumMainnet?.active || contractAddresses.ethereumMainnet?.current,
    "ethereumMainnet",
    "active",
  ),
]

export const proxyTableItems = activeEntries.filter(({ Type }) => Type === "proxy")

export const activeTableItems = activeEntries.filter(
  ({ Type }) => Type === "standalone"
)

export const implementationTableItems = [
  ...normalizeContractsForTable(contractAddresses.arbitrumOne?.currentImplementations, "arbitrumOne", "historical"),
  ...normalizeContractsForTable(contractAddresses.ethereumMainnet?.currentImplementations, "ethereumMainnet", "historical"),
]

export const nonActiveTableItems = [
  ...normalizeContractsForTable(contractAddresses.arbitrumOne?.paused, "arbitrumOne", "paused"),
  ...normalizeContractsForTable(contractAddresses.arbitrumOne?.migration_residual, "arbitrumOne", "migration_residual"),
  ...normalizeContractsForTable(contractAddresses.arbitrumOne?.legacy_operational, "arbitrumOne", "legacy_operational"),
  ...normalizeContractsForTable(contractAddresses.ethereumMainnet?.paused, "ethereumMainnet", "paused"),
  ...normalizeContractsForTable(contractAddresses.ethereumMainnet?.migration_residual, "ethereumMainnet", "migration_residual"),
  ...normalizeContractsForTable(contractAddresses.ethereumMainnet?.legacy_operational, "ethereumMainnet", "legacy_operational"),
]

export const currentTableItems = (() => {
  return [...activeTableItems, ...proxyTableItems, ...nonActiveTableItems]
})()
...
<SearchTable
  ...
  separatorColumn="Category"
  boldFirstColumn={true}
  columnVariant={{ "Chain": "textIcon", "Address": "addressWrapper" }}
    textIcons={[
    { label: "Arbitrum One", icon: <ArbitrumIcon color="var(--arbitrum)"/> },
    { label: "Ethereum Mainnet", icon: <Icon icon="ethereum" color="var(--hero-text)" size={13} /> }
  ]}
...
These are the currently deployed implementation targets behind upgradeable proxies. They are published for audit and upgrade-tracking workflows, but they are not part of the main active-only registry.

<SearchTable
  TableComponent={DynamicTable}
  headerList={["Name", "Address", "Type", "Chain"]}
  itemsList={implementationTableItems}
  columnWidths={{ Chain: '45%' }}
  monospaceColumns={[]}
  searchPlaceholder="Search current implementation contracts by name or address..."
  searchColumns={["Name", "Address"]}
  categoryColumn="Category"
  filterColumns={["Chain"]}
  showSeparators={true}
...
<SearchTable
  TableComponent={DynamicTable}
  headerList={["Name", "Address", "Type", "Status", "Chain"]}
  itemsList={nonActiveTableItems}
  columnWidths={{ Chain: '30%', Status: '22%' }}
  monospaceColumns={[]}
  searchPlaceholder="Search non-active contracts by name or address..."
  searchColumns={["Name", "Address"]}
  categoryColumn="Category"
  filterColumns={["Chain", "Status", "Type"]}
  showSeparators={true}
  separatorColumn="Category"
  boldFirstColumn={true}
  columnVariant={{ "Type": "badge", "Status": "badge", "Chain": "textIcon", "Address": "addressWrapper" }}
  categoryBadges={[
    { label: "standalone", color: "blue" },
    { label: "proxy", color: "surface" },
    { label: "Paused", color: "surface-destructive" },
    { label: "Migration complete", color: "yellow" },
    { label: "Migration residual", color: "yellow" },
...
<AccordionGroup>
  <Accordion title={<AccordionTitle icon={<Icon icon="gear" color="var(--accent)" />} title="Core" description="Historical staking, payments, round progression, and service discovery contracts" />}>
    <HistoricalContractTable contracts={["BondingManager","TicketBroker","Minter","RoundsManager","JobsManager","LivepeerVerifier"]} sourceData={contractAddresses} />
</Accordion>
  ...
  <Accordion title={<AccordionTitle icon={<Icon icon="landmark" color="var(--accent)" />} title="Governance" description="Historical voting and proposal contracts" />}>
    <HistoricalContractTable contracts={["BondingVotes"]} sourceData={contractAddresses} />
  </Accordion>
  ...
  <Accordion title={<AccordionTitle icon={<Icon icon="bridge" color="var(--accent)" />} title="Migration" description="Historical Confluence upgrade contracts" />}>
    <HistoricalContractTable contracts={["L2Migrator"]} sourceData={contractAddresses} />
  </Accordion>
</AccordionGroup>
