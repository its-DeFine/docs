...
const PACKAGE_JSON_PATHS = [
  ...
  'operations/scripts/snippets/generate-data/scripts/package.json',
]
...
function shouldExclude(repoPath) {
  ...
  if (p.startsWith('operations/scripts/archive/')) return true
  const parts = p.split('/')
...
function assignCategories(scriptPath, purpose, usages, template = {}) {
  ...
  if (
    scriptPath.startsWith('operations/scripts/test/') ||
...
function chooseCanonicalScript(paths) {
  function score(repoPath) {
    ...
    if (repoPath.startsWith('operations/scripts/')) value -= 10
    return value
  }
