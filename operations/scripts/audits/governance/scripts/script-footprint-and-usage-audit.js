...
function detectBackupArtifacts(issues, files) {
  files
    ...
    .forEach((file) => {
      const isScriptPath = file.relPath.startsWith('operations/scripts/') || file.relPath.startsWith('tests/');
      addIssue(issues, {
        id: 'backup-artifact',
...
function detectDuplicatePairs(issues, files) {
  const rootScripts = files
    ...
    .filter((file) => !file.relPath.startsWith('operations/scripts/test/'));
  ...
  for (const root of rootScripts) {
    ...
    const normalized = (value) =>
      String(value)
        .replace(/tools\/scripts\/test\//g, 'operations/scripts/')
        .replace(/tools\/scripts\//g, 'operations/scripts/')
        .trim();
