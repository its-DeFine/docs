...
const DEFAULT_REPORT_DIR = path.join(REPO_ROOT, 'operations', 'reports', 'health', 'page-links');
const DEFAULT_REPORT = path.join(DEFAULT_REPORT_DIR, 'page-links-audit.md');
const DEFAULT_REPORT_JSON = path.join(DEFAULT_REPORT_DIR, 'page-links-audit.json');
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (token === '--full') args.mode = 'full';
      ...
      args.outputDir = path.resolve(REPO_ROOT, argv[i + 1] || '');
  ...
  if (outputDirExplicit) {
    if (!reportExplicit) {
      args.report = path.join(args.outputDir, 'page-links-audit.md');
    }
    if (!reportJsonExplicit) {
      args.reportJson = path.join(args.outputDir, 'page-links-audit.json');
    }
  }
...
function extractRefs(content) {
  ...
  while ((m = markdownRegex.exec(contentForExtraction)) !== null) {
    ...
    const prefix = contentForExtraction.slice(0, m.index);
...
if (require.main === module) {
  runAudit({ argv: process.argv.slice(2) })
    ...
    .catch((error) => {
      console.error(`page-links-audit failed: ${error.message}`);
      process.exit(1);
    });
}
