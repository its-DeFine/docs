  ...
  if (stageId === 'cleanup-quarantine-manager') {
    const commands = [
      {
        ...
        command: 'node',
        args: ['operations/scripts/cleanup-quarantine-manager.js', '--output-dir', outputDirRel]
      }
    ];
    ...
    if (options.quarantine) {
      commands.push({
        label: `${stageId}:apply`,
        command: 'node',
        args: ['operations/scripts/cleanup-quarantine-manager.js', '--output-dir', outputDirRel, '--apply']
      });
    }
    ...
    return [
      {
        ...
        command: 'node',
        args: ['operations/scripts/cross-agent-packager.js', '--agent-pack', options.agentPack, '--output-dir', 'ai-tools/agent-packs']
      }
    ];
  }
