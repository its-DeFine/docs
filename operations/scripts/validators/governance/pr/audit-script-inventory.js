...
const SPECIAL_CALLERS = new Set(['.githooks/pre-commit', '.githooks/pre-push', 'operations/tests/run-all.js', 'tests/run-pr-checks.js']);
const PACKAGE_JSON_PATHS = ['tools/package.json', 'operations/tests/package.json'];
...
function isDirectHookTarget(targetPath) {
  return (
    ...
    targetPath.startsWith('operations/tests/unit/') ||
    targetPath.startsWith('operations/tests/integration/') ||
    targetPath === 'operations/tests/run-all.js' ||
    targetPath === 'tests/run-pr-checks.js' ||
    targetPath.startsWith('operations/scripts/') ||
...
function findTriggers(targetPath, docs, workflowDocs) {
  ...
  const runAll = docs.get('operations/tests/run-all.js');
  if (runAll && isDirectRunnerTarget(targetPath) && textReferencesPath(runAll, 'operations/tests/run-all.js', targetPath, 'module')) {
    triggers.push({ type: 'runner', caller: 'operations/tests/run-all.js', pipeline: 'P1' });
  }
...
function applyRepairPlan(plan) {
  ...
  if (plan.header_updates.length > 0 || plan.classification_changed) {
    const result = runNodeCommand(['operations/tests/unit/script-docs.test.js', '--write', '--rebuild-indexes']);
    if (result.status !== 0) {
