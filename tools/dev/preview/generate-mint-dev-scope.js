...
function buildScopedWorkspaceEntries(repoRoot, scopedDocs, scopedRoutes) {
  ...
  // Mint runtime expects this public snippets directory to exist when
  // composable page snippets are present in the scoped workspace.
  if ([...includedFiles].some((filePath) => filePath.startsWith('snippets/composables/pages/'))) {
    addWorkspaceEntry(entries, 'public/snippets/composables/pages/.keep', {
      type: 'content',
      content: ''
    });
  }
