...
const IGNORED_DIR_SEGMENTS = new Set(['node_modules', '.git', '.venv', 'tmp', 'notion']);
...
function usage() {
  console.log('Usage: node tools/scripts/audit-scripts.js [--format both|md|json] [--output-dir <dir>] [--strict]');
}
...
function fallbackPurpose(repoPath) {
  const name = path.basename(repoPath, path.extname(repoPath)).replace(/[-_.]+/g, ' ').trim();
  return `Utility script for ${name || repoPath}.`;
}

function tokenize(commandSegment) {
  return String(commandSegment || '').match(/"[^"]*"|'[^']*'|`[^`]*`|\S+/g) || [];
}
...
function resolvePackageDir(prefixToken, cwdRel) {
  ...
  if (!resolved) return '';
  if (resolved.endsWith('/package.json')) return normalizeRepoPath(path.dirname(resolved));
  return resolved;
}
...
function extractNpmRuns(segment, cwdRel, packageScriptsByDir) {
  ...
  while ((match = re.exec(segment))) {
    ...
    const packageDir = resolvePackageDir(prefixMatch ? prefixMatch[1] : '', cwdRel);
...
function collectScriptRefsFromCommand(command, cwdRel, scriptSet, packageScriptsByDir) {
  const collected = new Map();
  const seenNpm = new Set();

  function add(scriptPath, viaNpm, evidence) {
    const existing = collected.get(scriptPath) || { path: scriptPath, viaNpm: false, evidence: new Set() };
  ...
  function walk(currentCommand, currentDir, viaNpm, depth, chain) {
    ...
    for (const segment of segments) {
      ...
      for (const ref of directRefs) {
        add(ref, viaNpm, chain ? `${chain} -> ${compactText(segment)}` : compactText(segment));
      }
      ...
      for (const run of npmRuns) {
        ...
        const nestedCommand = packageData && packageData.scripts && packageData.scripts[run.scriptName];
...
function extractWorkflowTriggers(parsed, rawText) {
  ...
  const onValue = (() => {
    if (parsed && Object.prototype.hasOwnProperty.call(parsed, 'on')) return parsed.on;
    if (parsed && Object.prototype.hasOwnProperty.call(parsed, true)) return parsed.true;
    return null;
  })();
...
function buildUsageMap(scriptPaths, scriptSet, packageScriptsByDir) {
  ...
  for (const [packageDir, packageData] of packageScriptsByDir.entries()) {
    ...
    for (const scriptName of scriptNames) {
      ...
      if (!command) continue;
      const refs = collectScriptRefsFromCommand(command, packageDir, scriptSet, packageScriptsByDir);
      for (const ref of refs) {
        addUsage(usageMap, ref.path, {
          ...
          viaNpm: ref.viaNpm,
          evidence: [`command: ${compactText(command, 240)}`].concat(ref.evidence)
        });
      }
    }
  }
  ...
  if (fs.existsSync(workflowDirAbs) && fs.statSync(workflowDirAbs).isDirectory()) {
    ...
    for (const workflowName of workflowFiles) {
      const workflowPath = normalizeRepoPath(path.join(WORKFLOW_DIR, workflowName));
      ...
      for (const [jobId, jobDef] of Object.entries(jobs)) {
        ...
        const jobWorkingDir = normalizeRepoPath(String(job['working-directory'] || '').trim());
        const steps = Array.isArray(job.steps) ? job.steps : [];

        for (const step of steps) {
          if (!step || typeof step !== 'object' || typeof step.run !== 'string') continue;
          const stepName = String(step.name || 'run');
          const stepWorkingDir = normalizeRepoPath(String(step['working-directory'] || jobWorkingDir || '').trim());
          const refs = collectScriptRefsFromCommand(step.run, stepWorkingDir, scriptSet, packageScriptsByDir);
          for (const ref of refs) {
            ...
            addUsage(usageMap, ref.path, {
              ...
              viaNpm: ref.viaNpm,
              evidence: [`run: ${compactText(step.run, 240)}`].concat(ref.evidence)
            });
          }
        }
      }
  ...
  for (const hookPath of HOOK_FILES) {
    ...
    for (const cmd of commands) {
      const refs = collectScriptRefsFromCommand(cmd.command, '', scriptSet, packageScriptsByDir);
      for (const ref of refs) {
        ...
        addUsage(usageMap, ref.path, {
          ...
          whenTags: [...whenTags].sort(),
          viaNpm: ref.viaNpm,
          evidence: [`line: ${compactText(cmd.command, 240)}`].concat(ref.evidence)
        });
      }
    }
  }
...
function assignCategories(scriptPath, purpose, usages) {
  ...
  if (scriptPath.startsWith('.githooks/') || usages.some((entry) => entry.sourceType === 'hook') || hasAnyUsageWithTag(usages, 'pre-commit')) {
    roleTags.add('hook');
  ...
  if (/(enforce|verify|check|guard|lint|validate|validation|policy)/.test(lower) || hasAnyUsageWithTag(usages, 'pre-commit') || hasAnyUsageWithTag(usages, 'pr')) {
  ...
  if (/(run-all|run-pr|runner|entrypoint|test-suite)/.test(lower) || basename.startsWith('run-') || basename === 'lpd') {
    roleTags.add('runner');
    evidence.push('role:runner via orchestrator naming pattern');
  }

  if (scriptPath.startsWith('tools/scripts/test/') || /(fixture|stub|mock|allowed-test|allowed-script|allowed\.js)/.test(lower)) {
    roleTags.add('fixture');
    evidence.push('role:fixture via fixture/test-helper path or naming');
  }

  if (scriptPath.includes('.bak') || scriptPath.endsWith('.disabled') || /(deprecated|legacy)/.test(lower)) {
  ...
  if (scriptPath === '.githooks/pre-commit') {
    ...
    evidence.push('run-context:pre-commit for the primary pre-commit hook script');
  }
  ...
  if (roleTags.size === 0) {
    roleTags.add('helper');
    evidence.push('role:helper fallback because no stronger primary role was matched');
  }
...
function buildOverlap(scriptRecords) {
  ...
  exactClusters.sort((a, b) => {
    if (b.scripts.length !== a.scripts.length) return b.scripts.length - a.scripts.length;
  ...
  const recommendations = [...new Set(clusters.map((cluster) => cluster.recommendation))];
...
function summarizeUsage(entries) {
  if (!entries || entries.length === 0) return 'Manual/on-demand (no direct hook/workflow/package reference found)';
  const parts = entries.map((entry) => {
    const when = (entry.whenTags || []).length ? entry.whenTags.join(',') : 'manual';
    const where = entry.location ? `${entry.sourcePath}#${entry.location}` : entry.sourcePath;
...
function formatTemplateStatus(templateCompliance) {
  ...
  const parts = [];
  if (templateCompliance.missingTags.length) parts.push(`missing ${templateCompliance.missingTags.join(', ')}`);
  if (templateCompliance.emptyTags.length) parts.push(`empty ${templateCompliance.emptyTags.join(', ')}`);
...
function buildMarkdownReport(report) {
  ...
  lines.push('|---|---:|');
  for (const [tag, count] of Object.entries(summary.categoryCounts).sort((a, b) => a[0].localeCompare(b[0]))) {
  ...
  lines.push('');
  lines.push('| Script | Purpose | Template | Role Tags | Run Context Tags | Used and When |');
  lines.push('|---|---|---|---|---|---|');
  ...
  lines.push('');
  const failing = report.scripts.filter((script) => !script.templateCompliance.valid);
  if (failing.length === 0) {
    lines.push('All discovered scripts meet the required script template rules.');
  } else {
    for (const script of failing) {
      lines.push(`### \`${script.path}\``);
      lines.push(`- Missing tags: ${script.templateCompliance.missingTags.length ? script.templateCompliance.missingTags.join(', ') : 'none'}`);
      lines.push(`- Empty/placeholder tags: ${script.templateCompliance.emptyTags.length ? script.templateCompliance.emptyTags.join(', ') : 'none'}`);
      lines.push('');
    }
  }
  lines.push('');
  ...
  for (const script of report.scripts) {
    ...
    lines.push(`- Template compliance: ${formatTemplateStatus(script.templateCompliance)}`);
    lines.push(`- Role tags: ${script.categories.roleTags.join(', ')}`);
    lines.push(`- Run-context tags: ${script.categories.runContextTags.join(', ')}`);
    if (!script.usedBy.length) {
      lines.push('- Used by: Manual/on-demand (no direct hook/workflow/package reference found)');
    } else {
      lines.push('- Used by:');
      for (const usage of script.usedBy) {
        const when = usage.whenTags.length ? usage.whenTags.join(', ') : 'manual';
        const where = usage.location ? `${usage.sourcePath}#${usage.location}` : usage.sourcePath;
  ...
  if (!report.overlap.clusters.length) {
    ...
    for (const cluster of report.overlap.clusters) {
      ...
      lines.push(`- Scripts: ${cluster.scripts.map((value) => `\`${value}\``).join(', ')}`);
      lines.push(`- Canonical candidate: \`${cluster.canonical}\``);
      if (cluster.similarity !== undefined) {
        lines.push(`- Similarity: ${cluster.similarity}`);
      }
      lines.push(`- Shared role tags: ${cluster.sharedRoleTags.length ? cluster.sharedRoleTags.join(', ') : 'none'}`);
...
function buildReport(scriptRecords, overlap, options) {
  const { categoryCounts, roleTagCounts, runContextTagCounts } = buildCategoryCounts(scriptRecords);
  const templateCompliant = scriptRecords.filter((script) => script.templateCompliance.valid).length;
...
function main() {
  ...
  if (options.format === 'both' || options.format === 'md') {
    console.log(`Wrote: ${normalizeRepoPath(path.relative(REPO_ROOT, outputs.mdPath))}`);
  }
  if (options.format === 'both' || options.format === 'json') {
    console.log(`Wrote: ${normalizeRepoPath(path.relative(REPO_ROOT, outputs.jsonPath))}`);
  }

  if (options.strict && report.summary.templateNonCompliant > 0) {
    console.error(`Strict mode failure: ${report.summary.templateNonCompliant} scripts failed template compliance.`);
    process.exit(1);
  }
}
