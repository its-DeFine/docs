...
### Types (D-ACT-07, D-ACT-08)

All workflow YAML files are architecturally dispatchers. The type reflects what the script does, not the workflow's role. The type prefix stays in the filename for visual grouping in the flat `.github/workflows/` folder.

| Type          | Purpose                                        | Workflow count |
| ------------- | ---------------------------------------------- | -------------- |
| `integrator`  | Pulls external data into the repo (D-ACT-07)   | 12             |
...
| `interface`   | Reacts to issue/PR/external events (D-ACT-01)  | 5              |
