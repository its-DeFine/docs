...
description: 'Fetches Livepeer protocol contract addresses from governor-scripts, verifies against Arbiscan/Etherscan, and commits the canonical data files.'
pageType: reference
purpose: reference
audience: internal
status: active
...
graph LR
    ...
    subgraph "Data files produced"
        ...
        D3["_health-checks.json"]
        D4["livepeer-contract-addresses-data.json"]
    end
...
**Script:**
- `.github/scripts/fetch-contract-addresses.js` : Fetches addresses from governor-scripts, verifies against block explorers, writes all 4 data files

**Config:**
- `.github/scripts/contract-addresses-supplement.json` : Supplementary address data not in governor-scripts
...
- **P0 (from flags.jsonl):** This workflow has never been dispatched. It exists only on `docs-v2-dev`. GitHub Actions only indexes workflows on the default/production branch. Must cherry-pick workflow + fetch script to `docs-v2` then manual dispatch with `--dry-run` to verify
- **P2 (from audit):** `git add v2/` in commit step is overly broad (audit notes this but actual workflow uses targeted staging, so this may be stale audit data)
